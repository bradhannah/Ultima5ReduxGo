...
func NewRawEnemyReferences(gameConfig *config.UltimaVConfiguration) *rawEnemyReferences {
	...
	err := json.Unmarshal(additionalEnemyFlags, &allAdditionalEnemyFlags)
	if err != nil {
	...
	for nEnemy := 0; nEnemy < nTotalEnemies; nEnemy++ {
		...
		flagsBools := GetAsBitmapBoolList(enemyFlagsRaw, 0, nRawBytesPerEnemyFlag)
		...
		for nStat := 0; nStat < len(enemyStatsRaw); nStat++ {
			enemyRef.EnemyStats[enemyStat(nStat)] = int(enemyStatsRaw[nStat])
		}
